...
import { pricingFeatured, pricingTierPrices } from '../data/site';
      ...
      <div className="container-x relative">
        <div className="max-w-3xl mb-16">
          ...
          <h2 className="section-title">
            {t.pricing.titleA}
            <br />
            <span className="text-brand-400">{t.pricing.titleB}</span>
          </h2>
          <p className="mt-6 text-lg text-ink-100/75 max-w-xl">{t.pricing.body}</p>
        </div>

        <div className="grid md:grid-cols-3 gap-5">
          {t.pricing.tiers.map((tier, i) => {
            const featured = pricingFeatured[i];
            return (
              <motion.div
                key={tier.name}
              ...
              >
                ...
                <div className="mt-6 flex items-end gap-1">
                  ...
                  <span className="mb-2 text-sm text-ink-100/60">/ {t.pricing.per}</span>
                </div>
                    ...
                    <li key={f} className="flex items-start gap-3 text-sm text-ink-100/85">
                      <Check
                        ...
                        className={`mt-0.5 flex-none ${
                          featured ? 'text-brand-400' : 'text-brand-400/80'
                        }`}
                      />
                      <span>{f}</span>
                    </li>
        ...
        <p className="mt-10 text-center text-sm text-ink-100/60">{t.pricing.footnote}</p>
      </div>
    </section>
  );
}
